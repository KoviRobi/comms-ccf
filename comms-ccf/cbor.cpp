...
bool Cbor::packEmbedded(Major major, uint8_t value, std::span<uint8_t> & buf)
{
    if (buf.size() >= 1 && value <= EMBEDDED_MAX)
    {
        ...
        debugf(DEBUG "pack embedded %02X" END, START, buf[0]);
        buf = buf.subspan(1);
        return true;
    }
    return false;
...
std::optional<Cbor::Item> Cbor::unpack(std::span<uint8_t> & buf)
{
    if (buf.size() >= 1)
    {
        ...
        if (minor <= Minor::EmbeddedLast || minor == Minor::Indefinite)
        {
            ...
            debugf(DEBUG "unpack embedded %02X->%02X" END, START, initialByte, value);
            buf = buf.subspan(1);
        ...
        {
            uint8_t value = buf[1];
            debugf(DEBUG "unpack 8b %02X" END, START, value);
            buf = buf.subspan(2);
            return {{major, minor, value}};
        }
        else if (minor == Minor::TwoByteFollows && buf.size() >= 3)
        {
            ...
            debugf(DEBUG "unpack 16b %04X" END, START, value);
            buf = buf.subspan(3);
            return {{major, minor, value}};
        }
        else if (minor == Minor::FourByteFollows && buf.size() >= 5)
        {
            ...
            debugf(DEBUG "unpack 32b %08X" END, START, value);
            buf = buf.subspan(5);
            return {{major, minor, value}};
        }
