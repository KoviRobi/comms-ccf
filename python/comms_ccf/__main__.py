...
from comms_ccf.repl import Console, Stdio, repl, script
from comms_ccf.rpc import Rpc
from comms_ccf.tk import TkGui
...
async def init_locals(console: Console, rpc: Rpc):
    while True:
        try:
            await rpc.discover()
            break
        ...
        try:
            ...
            if args.log:
                ...
                if tk_gui is not None and tk_gui.logs is not None:
                    logs = t.cast(Table, tk_gui.logs)

                    def output(*row):
                        return logs.insert_row(row)

                background_tasks.add(print_logs, channels, output, args.expect_logs)

            background_tasks.add(channels.loop, args.debug)

            if args.repl or args.script_file:
                locals = await init_locals(console, rpc)
